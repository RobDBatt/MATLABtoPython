...
export function ConsentToggle({ className = '' }: { className?: string }) {
  ...
  return (
    <div className={`flex items-center gap-2 text-[11px] text-[#5a5f6b] ${className}`}>
      <button
        type="button"
        role="switch"
        aria-checked={on}
        onClick={toggle}
        className={`relative inline-flex h-4 w-7 shrink-0 items-center rounded-full transition-colors ${on ? 'bg-[#d9662b]' : 'bg-[#3a3f4d]'}`}
      >
      ...
      <span className="leading-snug">
        ...
        <Link href="/privacy/telemetry" className="underline underline-offset-2 hover:text-[#9aa1ac]">
          Learn more
        </Link>
      </span>
    </div>
